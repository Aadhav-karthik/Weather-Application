...
function FiveDayWeatherSection(props) {
    ...
    const DayRows = () => {
        ...
        for (let key in fiveDayWeatherData) {
            ...
            dayRows.push(
                <div style={daySelected} className="row text-md-start text-end weather-days" key={key} onClick={() => { dispatch(selectWeatherDay(key)) }}>
                    <div className="col-3 col-md-2">{new Date(key).toLocaleDateString("en-GB", { weekday: 'short', day: 'numeric' })}</div>

                    <div className="col-4 col-md-2">
                        <Tooltip title="Max and Min Temperatures" arrow><div>
                    ...
                    <div className="col-md-3 col-4 description">
                        <Tooltip title="Weather Description" arrow><div>{fiveDayWeatherData[key].description}</div></Tooltip>
                    </div>

                    <div className="col-md-2 col-4 pt-3 pt-md-0">
                        <Tooltip title="Humidity" arrow><div>{weatherIcon.HUMIDITY}{fiveDayWeatherData[key].humidity}%</div></Tooltip>
                    </div>
                    <div className="col-md-2 col-6 pt-3 pt-md-0">
                        <Tooltip title="Wind Speed" arrow><div>{weatherIcon.WIND}{fiveDayWeatherData[key].windSpeed} km/h</div></Tooltip>
                    </div>
                </div>)
        }
        return dayRows;
        ...
        <div className="col-md-6">
            <div className="card five-day-card">
                <div className="card-header five-day-header">
                    ...
                    <Tooltip title="City" arrow ><p className="city-header pt-2 d-inline-block">{city}</p></Tooltip>
